...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    test(c, INIT_POS, &[20, 400, 8_902, 197_281, 4_865_609]); //, 119_060_324]);
    test(c, KIWIPETE, &[48, 2_039, 97_862, 4_085_603]); //, 193_690_690]);
    test(c, POS3, &[14, 191, 2_812, 43_238, 674_624, 11_030_083]); //, 178_633_661]);
    test(c, POS4, &[6, 264, 9_467, 422_333, 15_833_292]);
    test(c, POS5, &[44, 1_486, 62_379, 2_103_487]); //, 89_941_194]);
    test(c, POS6, &[46, 2_079, 89_890, 3_894_594]); //, 164_075_551]);
}

fn test(c: &mut Criterion, pos: &str, expected: &[u64]) {
    c.bench_function(pos, |b| b.iter(|| {
        let board = Board::from_str(pos).expect(pos);
...
fn perft<const ROOT: bool>(board: &Board, depth: usize) -> u64 {
    ...
    for m in MoveGen::new_legal(board) {
        let this = board.make_move_new(m);
